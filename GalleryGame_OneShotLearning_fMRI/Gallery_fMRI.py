...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\emilo\\OneDrive - Lund University\\Experiment paradigms\\GalleryGame_fMRI\\Gallery_fMRI.py',
...
Randomization_loop = data.TrialHandler(nReps=1.0, method='random', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('OneShotLearning.xlsx'),
    seed=None, name='Randomization_loop')
thisExp.addLoop(Randomization_loop)  # add the loop to the experiment
thisRandomization_loop = Randomization_loop.trialList[0]  # so we can initialise stimuli with some values
# abbreviate parameter names if possible (e.g. rgb = thisRandomization_loop.rgb)
if thisRandomization_loop != None:
    for paramName in thisRandomization_loop:
        exec('{} = thisRandomization_loop[paramName]'.format(paramName))

for thisRandomization_loop in Randomization_loop:
    currentLoop = Randomization_loop
    # abbreviate parameter names if possible (e.g. rgb = thisRandomization_loop.rgb)
    if thisRandomization_loop != None:
        for paramName in thisRandomization_loop:
            exec('{} = thisRandomization_loop[paramName]'.format(paramName))
    ...
    for thisComponent in RandomizationComponents:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    t = 0
    _timeToFirstFrame = win.getFutureFlipTime(clock="now")
    RandomizationClock.reset(-_timeToFirstFrame)  # t0 is time of first possible flip
    frameN = -1
    
    # -------Run Routine "Randomization"-------
    while continueRoutine:
        # get current time
        t = RandomizationClock.getTime()
        tThisFlip = win.getFutureFlipTime(clock=RandomizationClock)
        tThisFlipGlobal = win.getFutureFlipTime(clock=None)
        frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
        # update/draw components on each frame
    ...
    routineTimer.reset()
# completed 1.0 repeats of 'Randomization_loop'
...
for thisTrial_1 in trials_1:
    ...
    Cued = data.TrialHandler(nReps=1.0, method='random', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions('OneShotLearning.xlsx', selection=Condition_rows),
        seed=None, name='Cued')
    ...
    for thisCued in Cued:
        ...
        # Select and record a value for listA
        thisA=listA.pop()

...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\emilo\\OneDrive - Lund University\\Jobb\\Experiments\\FacesNamesProffessions\\Encoding\\Encoding.py',
...
Circle = visual.ImageStim(
    win=win,
    name='Circle', 
    image='C:/Users/emilo/OneDrive - Lund University/Jobb/Experiments/FacesNamesProffessions/Circle.png', mask=None,
...
# Initialize components for Routine "Circle_y"
Circle_yClock = core.Clock()
...
fix_crossComponents = [polygon, Circle, key_resp_Circle]
...
while continueRoutine:
    ...
    # *Circle* updates
    if Circle.status == NOT_STARTED and tThisFlip >= (jitter/2)-0.5-frameTolerance:
        # keep track of start time/frame for later
    ...
    waitOnFlip = False
    if key_resp_Circle.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        # keep track of start time/frame for later
    ...
    if key_resp_Circle.status == STARTED:
        # is it time to stop? (based on global clock, using actual start)
        if tThisFlipGlobal > key_resp_Circle.tStartRefresh + 3-frameTolerance:
            # keep track of stop time/frame for later
...
thisExp.addData('key_resp_Circle.keys',key_resp_Circle.keys)
if key_resp_Circle.keys != None:  # we had a response
    thisExp.addData('key_resp_Circle.rt', key_resp_Circle.rt)
thisExp.addData('key_resp_Circle.started', key_resp_Circle.tStartRefresh)
thisExp.addData('key_resp_Circle.stopped', key_resp_Circle.tStopRefresh)
thisExp.nextEntry()
...
for thisName_trial in Name_trials:
    ...
    for thisEncoding_Name in Encoding_Name:
        ...
        for thisComponent in fix_crossComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        Encoding_Name.addData('Circle.started', Circle.tStartRefresh)
        Encoding_Name.addData('Circle.stopped', Circle.tStopRefresh)
        # check responses
        if key_resp_Circle.keys in ['', [], None]:  # No response was made
            key_resp_Circle.keys = None
        Encoding_Name.addData('key_resp_Circle.keys',key_resp_Circle.keys)
        if key_resp_Circle.keys != None:  # we had a response
            Encoding_Name.addData('key_resp_Circle.rt', key_resp_Circle.rt)
        Encoding_Name.addData('key_resp_Circle.started', key_resp_Circle.tStartRefresh)
        Encoding_Name.addData('key_resp_Circle.stopped', key_resp_Circle.tStopRefresh)
        # the Routine "fix_cross" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
        
        # ------Prepare to start Routine "Circle_y"-------
        continueRoutine = True
        ...
        _key_resp_3_allKeys = []
        # keep track of which components have finished
        Circle_yComponents = [key_resp_3, image_4]
        for thisComponent in Circle_yComponents:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        t = 0
        _timeToFirstFrame = win.getFutureFlipTime(clock="now")
        Circle_yClock.reset(-_timeToFirstFrame)  # t0 is time of first possible flip
        frameN = -1
        
        # -------Run Routine "Circle_y"-------
        while continueRoutine and routineTimer.getTime() > 0:
            # get current time
            t = Circle_yClock.getTime()
            tThisFlip = win.getFutureFlipTime(clock=Circle_yClock)
            tThisFlipGlobal = win.getFutureFlipTime(clock=None)
            frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
            # update/draw components on each frame
        ...
        Encoding_Name.addData('key_resp_3.keys',key_resp_3.keys)
        if key_resp_3.keys != None:  # we had a response
            Encoding_Name.addData('key_resp_3.rt', key_resp_3.rt)
        Encoding_Name.addData('key_resp_3.started', key_resp_3.tStartRefresh)
        Encoding_Name.addData('key_resp_3.stopped', key_resp_3.tStopRefresh)
        Encoding_Name.addData('image_4.started', image_4.tStartRefresh)
        Encoding_Name.addData('image_4.stopped', image_4.tStopRefresh)
        thisExp.nextEntry()
    ...
    for thisImmediateRecall_Name in ImmediateRecall_Name:
        ...
        if key_resp_3.keys in ['', [], None]:  # No response was made
            key_resp_3.keys = None
        ImmediateRecall_Name.addData('key_resp_3.keys',key_resp_3.keys)
        if key_resp_3.keys != None:  # we had a response
            ImmediateRecall_Name.addData('key_resp_3.rt', key_resp_3.rt)
        ImmediateRecall_Name.addData('key_resp_3.started', key_resp_3.tStartRefresh)
        ImmediateRecall_Name.addData('key_resp_3.stopped', key_resp_3.tStopRefresh)
        ImmediateRecall_Name.addData('image_4.started', image_4.tStartRefresh)
        ImmediateRecall_Name.addData('image_4.stopped', image_4.tStopRefresh)
        thisExp.nextEntry()
...
for thisProffession_trial in Proffession_trials:
    ...
    for thisEncoding_Proffession in Encoding_Proffession:
        ...
        for thisComponent in fix_crossComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        Encoding_Proffession.addData('Circle.started', Circle.tStartRefresh)
        Encoding_Proffession.addData('Circle.stopped', Circle.tStopRefresh)
        # check responses
        if key_resp_Circle.keys in ['', [], None]:  # No response was made
            key_resp_Circle.keys = None
        Encoding_Proffession.addData('key_resp_Circle.keys',key_resp_Circle.keys)
        if key_resp_Circle.keys != None:  # we had a response
            Encoding_Proffession.addData('key_resp_Circle.rt', key_resp_Circle.rt)
        Encoding_Proffession.addData('key_resp_Circle.started', key_resp_Circle.tStartRefresh)
        Encoding_Proffession.addData('key_resp_Circle.stopped', key_resp_Circle.tStopRefresh)
        # the Routine "fix_cross" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
        ...
        if key_resp_3.keys in ['', [], None]:  # No response was made
            key_resp_3.keys = None
        Encoding_Proffession.addData('key_resp_3.keys',key_resp_3.keys)
        if key_resp_3.keys != None:  # we had a response
            Encoding_Proffession.addData('key_resp_3.rt', key_resp_3.rt)
        Encoding_Proffession.addData('key_resp_3.started', key_resp_3.tStartRefresh)
        Encoding_Proffession.addData('key_resp_3.stopped', key_resp_3.tStopRefresh)
        Encoding_Proffession.addData('image_4.started', image_4.tStartRefresh)
        Encoding_Proffession.addData('image_4.stopped', image_4.tStopRefresh)
        thisExp.nextEntry()
    ...
    for thisImmediateRecall_Proffession in ImmediateRecall_Proffession:
        ...
        if key_resp_3.keys in ['', [], None]:  # No response was made
            key_resp_3.keys = None
        ImmediateRecall_Proffession.addData('key_resp_3.keys',key_resp_3.keys)
        if key_resp_3.keys != None:  # we had a response
            ImmediateRecall_Proffession.addData('key_resp_3.rt', key_resp_3.rt)
        ImmediateRecall_Proffession.addData('key_resp_3.started', key_resp_3.tStartRefresh)
        ImmediateRecall_Proffession.addData('key_resp_3.stopped', key_resp_3.tStopRefresh)
        ImmediateRecall_Proffession.addData('image_4.started', image_4.tStartRefresh)
        ImmediateRecall_Proffession.addData('image_4.stopped', image_4.tStopRefresh)
        thisExp.nextEntry()

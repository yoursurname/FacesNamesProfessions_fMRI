...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\Hassl\\OneDrive - Lund University\\Biofinder\\fMRI-tasks\\FacesNamesProfessions_fMRI\\FingerTapping\\Encoding_Recall_30-10_version_lastrun.py',
...
block = 0
n_images = 30
n_retrieval_images = 10
cross_duration_mean = 5
...
instructions = visual.TextStim(win=win, name='instructions',
    text='Nu är första halvan klar och det är dags för yrken. ',
    font='Open Sans',
    pos=(0, 0), height=0.07, wrapWidth=None, ori=0.0, 
    color='black', colorSpace='rgb', opacity=None, 
    languageStyle='LTR',
...
while continueRoutine and routineTimer.getTime() < 4.0:
    ...
    if memorera_text.status == STARTED:
        ...
        if tThisFlipGlobal > memorera_text.tStartRefresh + 4-frameTolerance:
...
Encoding = data.TrialHandler(nReps=1.0, method='random', 
    ...
    trialList=data.importConditions('Names_condition.xlsx'),
...
for thisEncoding in Encoding:
    ...
    ISI_duration = normal(cross_duration_mean,2.5)
    ...
    print(f"ISI duration: {ISI_duration}")
    # keep track of which components have finished
    ISIComponents = [nothing]
    ...
    while continueRoutine:
        ...
        # *nothing* updates
        if nothing.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
        ...
        if nothing.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > nothing.tStartRefresh + ISI_duration-frameTolerance:
                # keep track of stop time/frame for later
...
selection = randchoice(n_images, n_retrieval_images, False)

print(f"Selected rows for retrieval: {selection}")
# keep track of which components have finished
...
while continueRoutine and routineTimer.getTime() < 3.0:
    ...
    if instructions.status == STARTED:
        # is it time to stop? (based on global clock, using actual start)
        if tThisFlipGlobal > instructions.tStartRefresh + 3-frameTolerance:
            # keep track of stop time/frame for later
...
Encoding_prof = data.TrialHandler(nReps=1.0, method='random', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('Profession_condition.xlsx'),

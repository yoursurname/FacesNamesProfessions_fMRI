...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\Hassl\\OneDrive - Lund University\\Biofinder\\fMRI-tasks\\FacesNamesProfessions_fMRI\\FingerTapping\\Event_5_seconds_lastrun.py',
...
Encoding = data.TrialHandler(nReps=60.0, method='random', 
...
for thisEncoding in Encoding:
    ...
    cross_duration = normal(cross_duration_mean,2.5)
    
    if cross_duration > 10:
        cross_duration = 10
    if cross_duration < 2:
        cross_duration = 2
    ...
    while continueRoutine and routineTimer.getTime() < 4.0:
        ...
        if tap.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > tap.tStartRefresh + 4-frameTolerance:
                # keep track of stop time/frame for later
...
# completed 60.0 repeats of 'Encoding'

...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\Hassl\\OneDrive - Lund University\\Biofinder\\fMRI-tasks\\FacesNamesProfessions_fMRI\\Encoding_Recall\\Encoding_Recall_5_pair_version.py',
...
# --- Initialize components for Routine "block_parameters" ---
...
blocks = data.TrialHandler(nReps=6.0, method='sequential', 
    ...
    trialList=data.importConditions('Encoding_Recall.xlsx'),
...
for thisBlock in blocks:
    ...
    if Proffession == 1:
        instructions.setText("Nu är det dags för yrkestitlar")
    # keep track of which components have finished
    ...
    while continueRoutine and routineTimer.getTime() < 2.0:
        ...
        if instructions.status == STARTED:
            ...
            if tThisFlipGlobal > instructions.tStartRefresh + 2-frameTolerance:
    ...
    # keep track of which components have finished
    EncodeName_infoComponents = [text_9]
    ...
    # --- Prepare to start Routine "block_parameters" ---
    continueRoutine = True
    routineForceEnded = False
    # update component parameters for each repeat
    ...
    # keep track of which components have finished
    block_parametersComponents = []
    for thisComponent in block_parametersComponents:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    # --- Ending Routine "block_parameters" ---
    for thisComponent in block_parametersComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    ...
    print(f"Now the block is {block}")
    
    print(f'starting row: {start_row}\ending row: {end}')
    
    # the Routine "block_parameters" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    
    # set up handler to look after randomisation of conditions etc
    Encoding_names = data.TrialHandler(nReps=1.0, method='random', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions('Encoding_Recall.xlsx', selection=selection),
        seed=None, name='Encoding_names')
    thisExp.addLoop(Encoding_names)  # add the loop to the experiment
    thisEncoding_name = Encoding_names.trialList[0]  # so we can initialise stimuli with some values
    ...
    for thisEncoding_name in Encoding_names:
        currentLoop = Encoding_names
        # abbreviate parameter names if possible (e.g. rgb = thisEncoding_name.rgb)
        if thisEncoding_name != None:
            for paramName in thisEncoding_name:
                exec('{} = thisEncoding_name[paramName]'.format(paramName))
    ...
    # completed 1.0 repeats of 'Encoding_names'
    
    # get names of stimulus parameters
    if Encoding_names.trialList in ([], [None], None):
        params = []
    else:
        params = Encoding_names.trialList[0].keys()
    # save data for this loop
    Encoding_names.saveAsExcel(filename + '.xlsx', sheetName='Encoding_names',
        stimOut=params,
    ...
    # keep track of which components have finished
    RecallName_infoComponents = [text_6, fingers]
    ...
    # set up handler to look after randomisation of conditions etc
    Recall_names = data.TrialHandler(nReps=1.0, method='random', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions('Encoding_Recall.xlsx', selection=selection),
        seed=None, name='Recall_names')
    thisExp.addLoop(Recall_names)  # add the loop to the experiment
    thisRecall_name = Recall_names.trialList[0]  # so we can initialise stimuli with some values
    ...
    for thisRecall_name in Recall_names:
        currentLoop = Recall_names
        # abbreviate parameter names if possible (e.g. rgb = thisRecall_name.rgb)
        if thisRecall_name != None:
            for paramName in thisRecall_name:
                exec('{} = thisRecall_name[paramName]'.format(paramName))
        ...
        Recall_names.addData("correct", correct_answer)
        # check responses
        ...
        # store data for Recall_names (TrialHandler)
        Recall_names.addData('key_resp_1.keys',key_resp_1.keys)
        Recall_names.addData('key_resp_1.corr', key_resp_1.corr)
        if key_resp_1.keys != None:  # we had a response
            Recall_names.addData('key_resp_1.rt', key_resp_1.rt)
        # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
        if routineForceEnded:
            routineTimer.reset()
        else:
    ...
    # completed 1.0 repeats of 'Recall_names'
    
    # get names of stimulus parameters
    if Recall_names.trialList in ([], [None], None):
        params = []
    else:
        params = Recall_names.trialList[0].keys()
    # save data for this loop
    Recall_names.saveAsExcel(filename + '.xlsx', sheetName='Recall_names',
        stimOut=params,
        dataOut=['n','all_mean','all_std', 'all_raw'])
    thisExp.nextEntry()
